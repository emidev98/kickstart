...
beforeEach(async () => {
	...
	await factory.methods.createCampaign("100", "Build a spaceship").send({
		from: accounts[0],
		gas: 2000000,
		gasPrice: "10000000"
	});
	...
	campaign = await new web3.eth.Contract(JSON.parse(compiledCampaing.interface), campaignAddress);
});
...
describe("Campaigns", () => {
	...
	it("allows a manager to make a payment request", async () => {
		await campaign.methods.createRequest("Buy the rocket", "100", accounts[1]).send({
			from: accounts[0],
			gas: 2000000,
			gasPrice: "10000000"
		});
	...
	it("processes requests", async () => {
		...
		await campaign.methods.createRequest("Buy something else", web3.utils.toWei("5", "ether"), accounts[1]).send({
			from: accounts[0],
			gas: "200000",
			gasPrice: "10000000"
		});
